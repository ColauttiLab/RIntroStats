...
tDat<-data.frame(Pred1=X1, Pred2=X2) %>%
  mutate(Pred2mean=recode(Pred2, "Control"=0, "Treat1"=4)) %>%
  mutate(Resp= -1 + 0.3*Pred1 + Pred2mean + rnorm(N, sd=2))
```
...
$$ P = \frac{1}{1+e^{-(\beta_0 + \beta_1*X_1 + \beta_2X_2 + \epsilon)}} $$

```{r}
Logist<-1/(1+exp(-tDat$Resp))
qplot(x=tDat$Resp,y=Logist)
```
...
```{r}
tDat<-tDat %>%
  mutate(LResp=rbinom(n=N, size=1, prob=Logist))
```
...
$$ N = e^{\beta_0 + \beta_1*X_1 + \beta_2X_2 + \epsilon}$$

Similar to the logistic regression, we transform the model using this equation.

```{r}
Lambda<-exp(tDat$Resp)+1
tDat<-tDat %>% 
  mutate(PResp=rpois(N, lambda=Lambda))
```
